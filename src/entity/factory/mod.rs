use crate::component::{Component, Position, Size, Health};
...
impl FactoryEntities {
    ...
    pub fn get_entity_by_name(&mut self, name: String) -> Option<&mut Entity> {
        self.es.entities.iter_mut().find(|c| if c.name == name { true } else { false })
    }

    pub fn get_entity_by_id(&mut self, id: usize) -> Option<&mut Entity> {
        self.es.entities.iter_mut().find(|c| if c.id == id { true } else { false })
    }
